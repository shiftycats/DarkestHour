...
function UpdateRoles()
{
    ...
    for (i = 0; i < li_Roles.ItemCount; ++i)
    {
        ...
        if (PRI != none)
        {
            ...
            if (!PRI.IsInSquad() && GRI.GameType.default.bSquadSpecialRolesOnly && Limit != 255 && !RI.bExemptSquadRequirement)
            {
                S @= "*" $ SquadOnlyText $ "*";
            ...
            if (PRI.IsInSquad() && RI.bRequiresLeaderPosition && !PRI.IsLeaderPosition() && GRI.GameType.default.bSquadSpecialRolesOnly)
            {
